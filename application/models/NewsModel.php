...
class NewsModel extends Star_Model_Abstract
{
    ...
    public function getNewsInfoByPage($page, $page_size, Array $param)
    {
        ...
        if ($param['sort_flag']) {
            $select->limitPage($page, $page_size)->order(array('is_up DESC', 'time_create DESC', 'news_id DESC'));
        } else {
            $select->limitPage($page, $page_size)->order(array('is_up DESC', 'sort_id ASC', 'time_create DESC', 'news_id DESC'));
        }
